...
public enum UserServiceImplementation implements UserService {
    ...
    /**
     * Nombre d'iteration pour le calcul du hash du mot de passe.
     */
    private final static int ITERATIONNUMBER = 1000;
    ...
    public void authenticate(String login, String password) {
        ...
        byte[] proposedDigest = getHash(ITERATIONNUMBER, password, byteSalt);
        ...
        if (!isAllowedToPerformAction(user)) {
            ...
            builder4.with(SecurityOperations.SECURITY_EVENT,
                    user.getStatut() instanceof CompteBloquer ? SecurityEvent.USER_BLOCKED
                            : SecurityEvent.USER_INACTIVE);
    ...
    public User createUser(final String login, final String eMail, String password) {
        ...
        if (login != null && password != null) {
            try {
                ...
                byte[] byteDigest = getHash(ITERATIONNUMBER, password, byteSalt);
        ...
        if (user != null) {
            ...
            if (user.isActif()) {
                if (user.getNbrEssai() > 5) {
                    user.setStatut(new CompteBloquer());
                } else {
                    user.setStatut(new CompteActif(user));
                }
            } else {
                user.setStatut(new CompteInactif(user));
            }
    ...
    private boolean isAllowedToPerformAction(User user) {
        return user.isActif() && user.getStatut() instanceof CompteActif;
    }
}
